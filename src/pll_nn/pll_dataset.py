...
class DataSampler:
    ...
    def __init__(self, cfg):
        ...
        self.x, self.y = self.data_input_target_limited(self.data,
                                                        self.time)  # Convert the loaded data into input and target data with a time limit
        ...
        if self.cfg.dataset.validation_flag:
            self.x_train, self.x_val, self.x_test, self.y_train, self.y_val, self.y_test = self.train_val_test_split(
                self.x, self.y, self.split_ratio, True)
        else:
            self.x_train, self.x_test, self.y_train, self.y_test = self.train_val_test_split(self.x, self.y,
                                                                                             self.split_ratio, False)
    ...
    def define_train_val_data2(self, perc_of_data, perc_of_col_data, num_of_skip_data_points, num_of_col_points,
        ...
        x_train = self.x_train[:num_of_data: num_of_skip_data_points].clone().detach().to(self.device).requires_grad_(
            True)  # training data after skipping points
        y_train = self.y_train[:num_of_data: num_of_skip_data_points].clone().detach().to(self.device)
        x_train_col = self.x_train_col[:num_of_col_data: num_of_col_points].clone().detach().to(
            self.device).requires_grad_(True)  # traininng collaction points after skipping points
        x_train_col0 = self.x_train_col[self.x_train_col[:, 0] == 0].clone().detach().to(self.device).requires_grad_(
            True)  # ic traininng collaction points
        x_train_col0 = x_train_col0[:num_of_col_data].clone().detach().to(self.device).requires_grad_(
            True)  # ic traininng collaction points after skipping points
        y_train_col0 = x_train_col0[:, 1:].clone().detach().to(
            self.device)  # ic training collocation points (when time is 0)
        x_val = self.x_val[:: num_of_skip_val_points].clone().detach().to(self.device).requires_grad_(
            True)  # validation data without skipping points
        y_val = self.y_val[:: num_of_skip_val_points].clone().detach().to(self.device)
        self.training_shape, self.training_col_shape, self.training_col_shape0, self.validation_shape = (
            x_train.shape[0], x_train_col.shape[0], x_train_col0.shape[0], x_val.shape[0])
        if self.cfg.nn.type == "PinnA":
            y_train = y_train[x_train[:, 0] != 0].clone().detach().to(
                self.device)  # remove the 0 time  from the output data
            x_train = x_train[x_train[:, 0] != 0].clone().detach().to(self.device).requires_grad_(
                True)  # remove the 0 time  from the input data
            x_train_col = x_train_col[x_train_col[:, 0] != 0].clone().detach().to(self.device).requires_grad_(
                True)  # remove the 0 time from the input data
            # keep only the columns without time

        x_train = self.transform_input(
            x_train)  # transform the input data according to the respective chosen input_transform method
        x_train_col = self.transform_input(
            x_train_col)  # transform the input data according to the respective chosen input_transform method
        x_val = self.transform_input(
            x_val)  # transform the input data according to the respective chosen input_transform method
        x_train_col0 = self.transform_input(
            x_train_col0)  # transform the input data according to the respective chosen input_transform method
    ...
    def define_test_data(self, starting_traj, sample_per_traj, total_traj):
        x_test = self.x_test[
                 starting_traj * sample_per_traj:(starting_traj + total_traj) * sample_per_traj].clone().detach().to(
    ...
    def train_val_test_split(self, x_data, y_data, split_ratio, val_flag=True):
        ...
        split = int(
            self.total_trajectories * split_ratio * self.sample_per_traj)  # split trajectories and not just points into train, val and test
        if val_flag:
            val_split = int(self.total_trajectories * (10 - split_ratio * 10) / (
                        2 * 10) * self.sample_per_traj)  # multiply by 10 both denominator and numerator to get int values for sure
            ...
            print("Number of training samples: ", len(x_train), "Number of validation samples: ", len(x_val),
                  "Number of testing samples: ", len(x_test))
            self.total_test_trajectories = int(self.total_trajectories * (10 - split_ratio * 10) / (
                        2 * 10))  # number of test trajectories is given by the val_split and is equal to val trajectories
    ...
    def create_col_points(self):
        """
        Create collocation points for PLL-ROM by randomly sampling initial
        (δ₀, ω₀) pairs within configured ranges, then repeating each pair
        along the time axis.

        Returns:
            torch.Tensor of shape (M * T, 1 + 2), where
              M = number of samples (n_samples),
              T = number of time points (num_of_points),
        ...
        ic_cfg = OmegaConf.load(yml_files[0])

        # 2) read sampling parameters from config
        M = getattr(self.cfg.dataset, 'n_samples', None) \
            or getattr(self.cfg.dataset, 'num_collocation_groups', 10)

        # 3) sample δ₀ and ω₀ uniformly in [min, max]
        d0_min, d0_max = ic_cfg.delta0.min, ic_cfg.delta0.max
        w0_min, w0_max = ic_cfg.omega0.min, ic_cfg.omega0.max
        d0 = np.random.uniform(d0_min, d0_max, size=M)
        w0 = np.random.uniform(w0_min, w0_max, size=M)
        init_cond = np.stack([d0, w0], axis=1)  # shape (M, 2)

        # 4) build time vector
        T = self.cfg.num_of_points
        time = torch.linspace(0, self.time, T).unsqueeze(1)  # shape (T,1)

        # 5) for each (δ₀, ω₀), repeat it T times and prepend time column
        col_list = []
        for delta_i, omega_i in init_cond:
            # create a (1,2) tensor [δ₀, ω₀] then repeat to (T,2)
            ic_vec = torch.tensor([[delta_i, omega_i]], dtype=torch.float32)
            rep = ic_vec.repeat(T, 1)  # (T,2)
            # concatenate time column to get (T,3)
            col_list.append(torch.cat([time, rep], dim=1))

        # 6) stack all M trajectories into (M*T, 3)
        col_points_tensor = torch.cat(col_list, dim=0)
